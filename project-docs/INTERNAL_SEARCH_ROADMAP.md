...
- **Guided results framing.** When a known operational intent is recognized, render a top card explaining which cluster should handle the problem (workflow first, then blueprint, then comparison), using the same cluster discipline from CLAUDE.md.
