def render_stack_blueprint(spec, context):
    ...
    stack_boxes = [
        (198, 630, 786, 724, palette["accent"]),
        (232, 524, 752, 616, palette["secondary"]),
        (272, 418, 712, 510, palette["warm"]),
    ]
    for idx, (x1, y1, x2, y2, fill) in enumerate(stack_boxes):
        ...
        draw.text((x1 + 28, y1 + 18), layer["title"], font=fonts["card_title"], fill="#f7fbfd")
        helpers["draw_text_block"](draw, layer["detail"], fonts["small"], "#e6eef7", (x1 + 28, y1 + 52, (x2 - x1) - 56, 48), 4, report, f"stack layer {layer['title']}", max_lines=2)
