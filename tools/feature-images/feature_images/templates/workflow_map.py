def render_workflow_map(spec, context):
    ...
    for index, item in enumerate(items):
        ...
        helpers["pill"](draw, (x + 24, top_y + 24, x + 122, top_y + 62), palette["frame"], item["kicker"], fonts["small"], palette["muted"])
        draw.text((x + 24, top_y + 84), item["label"], font=fonts["card_title"], fill=palette["ink"])
        helpers["draw_text_block"](
            draw,
            item["detail"],
            fonts["card_body"],
            palette["muted"],
            (x + 24, top_y + 124, card_width - 48, 64),
            6,
            report,
        ...
        if index < len(items) - 1:
            ...
            draw.ellipse((x + card_width + gap - 12, connector_y - 12, x + card_width + gap + 12, connector_y + 12), fill=palette["accent"])

    note_box = (150, 640, 760, 760)
    signal_box = (790, 640, 1450, 760)
    ...
    draw.text((182, 674), spec["supporting_note"]["title"], font=fonts["card_title"], fill=palette["ink"])
    helpers["draw_text_block"](draw, spec["supporting_note"]["body"], fonts["card_body"], palette["muted"], (182, 718, 530, 60), 6, report, "workflow supporting note", max_lines=2)
    draw.text((822, 674), spec["signal_note"]["title"], font=fonts["card_title"], fill=palette["ink"])
    helpers["draw_text_block"](draw, spec["signal_note"]["body"], fonts["card_body"], palette["muted"], (822, 718, 550, 60), 6, report, "workflow signal note", max_lines=2)
