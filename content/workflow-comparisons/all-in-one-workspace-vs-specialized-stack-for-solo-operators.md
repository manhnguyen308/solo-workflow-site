...
| Question | Consolidated model | Specialized model |
|---|---|---|
...
## Stay consolidated when the record still works

A consolidated model remains defensible when:

- one person owns the relevant decisions;
- current status and next action are visible in one review;
- the missing feature is a convenience rather than a requirement;
- the workflow is changing and would force repeated reconfiguration;
- another tool would duplicate an existing record.

Document conventions so flexibility does not turn into inconsistent project structures.

## Specialize one function at a time

A specialized tool has earned a role when:

- the current system cannot support a required permission, record, control, or handoff;
- the missing capability is confirmed in current product documentation;
- the new tool will replace a workaround or existing subscription;
- one person owns the transfer and exception path;
- authoritative fields are divided explicitly.

Complete this sentence before adding it:

> The specialist tool owns ________. The main operating record remains authoritative for ________.

If the answer assigns the same fact to both systems, revise the design.

## Test the handoff

Use a representative workflow event, such as signed agreement, deliverable approval, invoice sent, or payment received. Verify:

1. what starts the transfer;
2. which fields or files move;
3. who confirms completion;
4. where an exception is recorded;
5. which system answers the next weekly question.

A specialized stack has failed when the operator must compare two values before knowing the current state.

## Continue from the decision

- Build a consolidated baseline with the [lean software stack blueprint](/guides/software-stack-blueprint-solo-freelancer-lean-budget/).
- Apply a purchase test with [how to choose a stack without overbuying](/guides/how-to-choose-a-software-stack-without-overbuying-tools/).
