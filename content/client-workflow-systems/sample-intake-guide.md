...
Lead collection is incomplete until each inquiry receives a routing decision.
...
Use a simple fit rubric and define what each label means for your service:

| Dimension | Not a fit | Needs review | Ready to proceed |
|---|---|---|---|
| Service fit | Outside scope | Partial fit | Matches the offered service |
| Budget fit | Below the stated floor | Budget needs clarification | Within the stated range |
| Timeline realism | Conflicts with a firm constraint | Feasible only if assumptions change | Fits available capacity and dependencies |
| Decision readiness | Gathering general information | Stakeholders or criteria are unclear | Decision path and next step are known |

Reject or nurture a lead when a non-negotiable criterion is not met. Review mixed results manually. Book discovery when the required fit criteria are met and the call has a defined purpose.

## Step 3: add decision tree logic

- If a required budget or timeline condition is not met, reject the inquiry.
- If the service fits but the timing does not, nurture the lead for later.
- If the required fit criteria are met, offer discovery booking.
...
Revise the intake rules if:
- you regularly make exceptions for leads that miss a non-negotiable fit criterion,
...
## Intake mistakes and corrections

- Asking questions without using the answers: remove fields not tied to qualification criteria.
- Booking every lead before applying fit criteria: apply the documented criteria before scheduling.
- Mixing intake and onboarding details: keep intake focused on the fit decision, then use the onboarding checklist after signing.
