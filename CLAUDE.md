...
Read this file first for every Claude Code or Codex pass in this repo. It is the single file of durable operating rules for working on `soloopsguide.com`; it replaced `AGENT.md` on 2026-09-24. Codex loads `AGENTS.md` by default, not this file, so start Codex passes by telling it to read `CLAUDE.md`.
...
- Cloudflare Pages should remain pinned to `HUGO_VERSION=0.160.1`. Local Snap installs may drift, so always report the exact `hugo version` used for validation. The legacy vendored binary `tools/hugo/v0.128.0/hugo.exe` is a Windows PE32+ binary and is no longer the source of truth.
...
Read `CLAUDE.md` + `TRACKER.md` before every task. Add these for specific work types:
